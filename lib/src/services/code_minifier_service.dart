class CodeMinifierService {
  static String minify(String code) {
    ...
    code = code.replaceAll(RegExp(r'\s+'), ' ');  // Replace multiple spaces with one
    code = code.replaceAll(RegExp(r'\s*\{\s*'), '{');  // Remove space before opening brace
    code = code.replaceAll(RegExp(r'\s*\}\s*'), '}');  // Remove space after closing brace
    code = code.trim();  // Remove leading/trailing spaces
    
    // 4. Minimize `if (x == true)` to `if (x)` for simplicity
    code = code.replaceAll(RegExp(r'if\s*\(\s*(\w+)\s*==\s*true\s*\)'), r'if($1)');
    
    // 5. Remove redundant `else` after `return` statement
    code = code.replaceAll(RegExp(r'return\s*[^;]+;\s*else\s*{'), r'return $1;');
